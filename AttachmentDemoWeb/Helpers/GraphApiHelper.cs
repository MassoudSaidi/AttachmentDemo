...
namespace AttachmentDemoWeb.Helpers
{
    ...
    {
        internal static async Task<HttpResponseMessage> ForwardMessageBackEnd(string accessToken, SaveAttachmentRequest request)
        {
            try
            {
                ...
                var toRecipients = new List<Recipient>()
                {
                    new Recipient
                    {
                        ...
                        {
                            Name = "Org Box",
                            Address = "sharedBox@sdrcd.onmicrosoft.com"
                        }
                    }
                };

                //await graphClient.Me.Messages[request.messageId] also didn't work
                //await graphClient.Users["33b6f30cf5ef4d5f"].Messages[request.messageId]
                /*await graphClient.Me.Messages[request.messageId]
